...
set_light () {
    kitty +kitten themes --config-file-name theme.conf --reload-in=all Edge-light && pkill -SIGUSR1 kitty &
...
set_dark () {
    kitty +kitten themes --config-file-name theme.conf --reload-in=all Edge-neon && pkill -SIGUSR1 kitty &
